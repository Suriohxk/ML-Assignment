...
st.set_page_config(
    page_title="Scribe · AI Blog Generator",
    page_icon="✦",
    layout="wide",
...
[data-testid="stSidebar"] h2 { color: #e8e8e8; font-size: 1.4rem; letter-spacing: 0.04em; }
...
.download-row { margin-top: 0.8rem; }
</style>
...
with st.sidebar:
    st.markdown("## ✦ Scribe")
    st.caption("AI-powered blog generator · Gemini + LangGraph")
    st.divider()

    topic   = st.text_area("✏️ Topic", placeholder="e.g. How transformers work", height=110,
                            help="Describe the subject you want a blog post about.")
    as_of   = st.date_input("📅 As-of date", value=date.today(),
                            help="Used to gauge how recent the content should be.")
...
st.markdown("# ✦ Scribe")
st.caption("Enter a topic in the sidebar and click **⚡ Generate** to produce a full technical blog post.")

tab_article, tab_plan, tab_sources, tab_images, tab_logs = st.tabs([
    "📄 Article", "📋 Plan", "🔍 Sources", "🖼️ Images", "📜 Logs"
])
...
if out:
    ...
    with tab_article:
        if not final_md:
            ...
            m2.metric("Read time", reading_time(wc))
            m3.metric("Sections", nsec)
            m4.metric("Images",   nim)
            st.divider()
            ...
            c1, c2 = st.columns(2)
            c1.download_button("⬇️ Download .md", data=final_md.encode(),
    ...
    with tab_logs:
        ...
        if all_logs:
            st.caption(f"{len(all_logs)} events recorded")
            st.text_area("", value="\n\n".join(all_logs[-80:]), height=480, label_visibility="collapsed")
            if st.button("🗑️ Clear logs"):
                st.session_state["run_logs"] = []
                st.rerun()
        else:
            st.info("No logs yet. Generate an article to see pipeline events.")

else:
    with tab_article:
        st.info("Enter a topic in the sidebar and click **⚡ Generate** to get started.")
